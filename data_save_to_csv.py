...
def connet_database():
	...
	conn = pymysql.connect(host="192.168.254.129", port=3306, user="alan", passwd="alan", db="mit_image", charset="utf8")
	cur = conn.cursor()
	return conn, cur
...
def xml_parser(file_path, cursor, conn):
	...
	for object in object_list:
		...
		for point in point_list:
			...
			# sql = 'insert feature () value (' + str(file_name).replace('\n', '') + ',' + str(name).replace('\n', '')
			# + ',' + count + ',' + int(str(point_x).replace('\n', '')) + ',' + int(str(point_y).replace('\n', '')) + ')'
			sql = 'insert into feature(img_name, feature_name, feature_num, cordinate_x, cordinate_y) values' + str(insert_data)
